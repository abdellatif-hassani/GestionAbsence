...
function inscriptionEtudiant($id_user, $cin, $nom, $cne, $prenom, $date_naissance, $adresse, $lieu_naissance, $telephone, $email,$idClass)
{

    $db = database();

    $sttm = $db->prepare("INSERT INTO etudiant (id_user, cin, nom, cne, prenom, date_naissance, adresse, lieu_naissance, telephone, email,idClass) 
                            VALUES (:id_user, :cin, :nom, :cne, :prenom, :date_naissance, :adresse, :lieu_naissance, :telephone, :email, :idClass)");
    ...
    $sttm->bindParam(':idClass', $idClass);
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["type"]) && !empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["email"]) && !empty($_POST["telephone"])) {
            ...
            if ($id_user != null) {
                if ($_POST["type"] == "etudiant") {
                    ...
                    if (inscriptionEtudiant($id_user, $_POST["cin"], $_POST["nom"], $_POST["cne"], $_POST["prenom"], $_POST["date_naissance"], $_POST["adresse"], $_POST["lieu_naissance"], $_POST["telephone"], $_POST["email"],$_POST["idClass"])) {
...
function toutEtudiants()
{
    ...
    $sttm = $db->prepare("SELECT * FROM etudiant  e
                        LEFT JOIN class c
                        ON e.idClass=c.idClass");
    if ($sttm->execute()) {
        $students = $sttm->fetchAll();
        return $students;
    }
...
// la fonction qui retourne la liste des étudiants d'une classe 
function toutEtudiantsClass($idClass)
{
...
function toutClasses()
{
    ...
    $sttm = $db->prepare("SELECT * FROM class");
    if ($sttm->execute()) {
        $classes = $sttm->fetchAll();
        return $classes;
    }
...
function listAbsenceParEtudiant($id)
{
    ...
    $sttm = $db->prepare("SELECT * FROM absence where id_etudiant=:id and is_old=0 ORDER BY date_absence DESC");
    $sttm->bindParam(":id", $id);
...
function listAbsences()
{
    ...
    $sttm = $db->prepare("SELECT * FROM absence where is_old=0 ORDER BY date_absence DESC");
    ...
    if ($sttm->execute()) {
        ...
        foreach ($absences_ as $a) {
            ...
            $sttm3 = $db->prepare("SELECT * FROM professeur where id_user=:id");
...
function listAbsencesOld()
{
    ...
    $sttm = $db->prepare("SELECT * FROM absence 
                        where is_old=0
...
function listerAbsencesParProf()
{
    ...
    */
    ...
    $sttm = $db->prepare("SELECT * FROM absence where professeur=:id and is_old=0 ORDER BY date_absence DESC");
...
function update_student($etd)
{
    ...
    $sttm = $db->prepare("UPDATE etudiant
        ...
        SET utilisateur.login = :login, utilisateur.password = :password, etudiant.nom = :nom,
            etudiant.prenom = :prenom, etudiant.cin = :cin, etudiant.cne = :cne, etudiant.email = :email,
            etudiant.date_naissance = :date_naissance, etudiant.adresse = :adresse, 
            etudiant.lieu_naissance = :lieu_naissance, etudiant.telephone = :telephone,
            etudiant.idClass = :idClass
    ...
    $sttm->bindParam(':idClass', $etd['idClass']);
...
function getEtudiantToModify($id)
{
    ...
    $student = "";
    $sttm = $db->prepare("SELECT etudiant.*, utilisateur.login, utilisateur.password,libelleClass
            FROM etudiant 
            LEFT JOIN class 
            ON etudiant.idClass=class.idClass
...
//ajouter une class 
function ajoutClassFunction($libelle,$nbEtudiants,$annee){
    $db = database();

    $sttm = $db->prepare("INSERT INTO class VALUES ('','$libelle',$nbEtudiants,'$annee')");

    if ($sttm->execute()) {
        header("Location: ajoutClass.php?addClass=done");
    }
}

function getInfoClass($idClass){
    $db = database();

    $class = "";
    $sttm = $db->prepare("SELECT * FROM class
            WHERE idClass = :id;");
    $sttm->bindParam(':id', $idClass);
    if ($sttm->execute()) {
        $class = $sttm->fetch(PDO::FETCH_ASSOC);
        return $class;
    }
    return $class;
}

function modifierClass($idClass,$libelle,$nbEtudiants,$annee){
    $db = database();

    $sttm = $db->prepare("UPDATE class SET libelleClass='$libelle',nbEtudiants='$nbEtudiants',annee='$annee'
                          WHERE idClass=:id");
    $sttm->bindParam(":id", $idClass);
